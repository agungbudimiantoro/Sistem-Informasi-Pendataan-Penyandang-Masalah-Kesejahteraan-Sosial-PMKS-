<h3 class="text-center text-capitalize mb-3">edit Data tindakan</h3>
<form action="?p=tindakan_proses" method="POST" enctype="multipart/form-data">
    <?php
    $no = 1;
    $id = $_GET['id'];

    $query = mysqli_query($conn, "SELECT * FROM tindakan, subjek, subjek_kedua, dt_pmks where subjek.id_subjek = subjek_kedua.id_subjek and subjek.id_subjek = tindakan.id_subjek
  and tindakan.id_dt_pmks = dt_pmks.id_dt_pmks and tindakan.id_tindakan='$id'");
